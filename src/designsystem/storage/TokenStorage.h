...
public:
    ...
    std::optional<ComponentToken> getComponent(const TokenPath& path) const;
    
    // Check if token exists (any type)
    bool hasToken(const TokenPath& path) const;
    
    // Get token category (for editor)
    QString getTokenCategory(const TokenPath& path) const;
...
private:
    ...
    // Component storage
    std::unordered_map<TokenPath, ComponentToken> m_componentTokens;
